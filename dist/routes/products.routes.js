...
async function favoriteForProduct(userId, productId, odooFavorite) {
...
router.get('/', async (req, res) => {
    try {
        ...
        const data = products.map(product => {
            const favorite = typeof product.__favorite === 'boolean'
                ? product.__favorite
...
router.get('/tags', async (req, res) => {
    try {
        ...
        const message = error instanceof Error ? error.message : 'Failed to load product tags.';
...
router.post('/', async (req, res) => {
    try {
        ...
        const type = typeRaw === 'service' || typeRaw === 'combo'
            ? typeRaw
            : 'consu';
        ...
        const created = await createOdooProduct(req.user.id, {
            name,
            type,
            saleOk: typeof body.saleOk === 'boolean'
                ? body.saleOk
                : body.sale_ok !== undefined
                    ? Boolean(body.sale_ok)
                    : undefined,
            purchaseOk: typeof body.purchaseOk === 'boolean'
                ? body.purchaseOk
                : body.purchase_ok !== undefined
                    ? Boolean(body.purchase_ok)
                    : undefined,
            trackInventory: typeof body.trackInventory === 'boolean'
                ? body.trackInventory
                : undefined,
            invoicePolicy: body.invoicePolicy === 'delivery' || body.invoicePolicy === 'order'
                ? body.invoicePolicy
                : undefined,
            ...
            websitePublished: typeof body.websitePublished === 'boolean'
            ...
            sellWhenOutOfStock: typeof body.sellWhenOutOfStock === 'boolean'
                ? body.sellWhenOutOfStock
                : undefined,
            showAvailableQty: typeof body.showAvailableQty === 'boolean'
                ? body.showAvailableQty
                : undefined,
            outOfStockMessage: body.outOfStockMessage != null
                ? String(body.outOfStockMessage)
                : undefined,
            longDescription: body.longDescription != null ? String(body.longDescription) : undefined,
        });
...
router.get('/:id/image', async (req, res) => {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : 'Failed to load product image.';
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        const favorite = await favoriteForProduct(req.user.id, product.id, product.__favorite);
        let premium = {
            pricelistId: null,
        ...
        try {
            ...
            console.warn('[products] membership prices unavailable:', priceError instanceof Error ? priceError.message : priceError);
        }
        let appAccess = null;
        try {
            appAccess = mapAppAccess(await fetchOdooProductAppAccess(req.user.id, product.id));
        }
        catch (appError) {
            console.warn('[products] app access unavailable:', appError instanceof Error ? appError.message : appError);
        }
        return res.json({
            data: {
                id: String(product.id),
...
router.put('/:id/app', async (req, res) => {
    const productId = Number(req.params.id);
    if (!Number.isFinite(productId) || productId <= 0) {
        return res.status(400).json({ message: 'Invalid product id.' });
    }
    const body = req.body ?? {};
    const enableQrApp = typeof body.enableQrApp === 'boolean' ? body.enableQrApp : undefined;
    const websitePublished = typeof body.websitePublished === 'boolean'
        ? body.websitePublished
        : undefined;
    ...
    try {
        ...
        if (/session expired/i.test(message))
            status = 401;
        else if (/not found|invalid product/i.test(message))
            status = 404;
        else if (/no app settings/i.test(message))
            status = 400;
...
router.put('/:id/prices', async (req, res) => {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : 'Failed to update product prices.';
...
router.put('/:id/favorite', async (req, res) => {
    ...
    try {
        const wroteToOdoo = await updateOdooProductFavorite(req.user.id, productId, favorite);
        if (!wroteToOdoo) {
